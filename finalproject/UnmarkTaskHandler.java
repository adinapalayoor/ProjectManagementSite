...
import com.nildesperandumcs3733.finalproject.http.UnmarkTaskRequest;
import com.nildesperandumcs3733.finalproject.http.UnmarkTaskResponse;
 ...
 * @author heineman
 */
public class UnmarkTaskHandler implements RequestHandler<UnmarkTaskRequest,UnmarkTaskResponse> {
	 ...
	 * @throws Exception 
	 */
	boolean unmarkTask(String task, String projectID) throws Exception {
		 
			if (logger != null) { logger.log("in unmarkTask"); }
			TaskDAO dao = new TaskDAO();
			// check if present
			Task exist =dao.getTask(task, projectID);
			if (exist != null) {
				return dao.unmarkTask(exist);
			} else {
	...
	@Override
	public UnmarkTaskResponse handleRequest(UnmarkTaskRequest req, Context context) {
		...
		boolean fail= false;
		try {
			fail = unmarkTask(req.getTaskID(),req.getProjectID());
		} catch (Exception e) {
		...
		UnmarkTaskResponse response;
		if (!fail) {
			response = new UnmarkTaskResponse(400, failMessage);
		} else {
			response = new UnmarkTaskResponse("Task Unmarked", 200);  // success
		}
		return response; 
	}
}
